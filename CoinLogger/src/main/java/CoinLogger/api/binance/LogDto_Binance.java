...
@Getter
@Setter
public class LogDto_Binance implements LogDto {
    ...
    public  LogDto_Binance (JSONObject jsonObject, double dollar ) {
        Instant instant = Instant.ofEpochMilli((Long) jsonObject.get("time")); // 밀리초를 Instant으로 변환
        LocalDateTime dateTime = LocalDateTime.ofInstant(instant, ZoneId.systemDefault()); // Instant을 LocalDateTime으로 변환
